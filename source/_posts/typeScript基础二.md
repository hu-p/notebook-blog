...
TypeScript 3.4 还引入了对只读元组的新支持。我们可以**为任何元组类型加上 `readonly` 关键字前缀，以使其成为只读元组**。具体的示例如下：
...
//表示mayNullOrUndefinedOrString不为空，故而类型为string
...
```js
{
  //由于const定义常量，此处定义的类型为this is string，尽管typeof的结果为string，但其实其类型推断为'this is string'
  //下方类似
...
```js
{
    //let为变量，故缺省显式类型注解的可变更的变量的类型转换为了赋值字面量类型的父类型，类型显示正常
...
所有通过 let 或 var 定义的变量、函数的形参、对象的非只读属性，如果满足指定了**初始值且未显式添加类型注解的条件，那么它们推断出来的类型就是指定的初始值字面量类型拓宽后的类型，这就是字面量类型拓宽**。
...
比如对 null 和 undefined 的类型进行拓宽，通**过 let、var 定义的变量如果满足未显式声明类型注解且被赋予了 null 或 undefined 值，则推断出这些变量的类型是 any**：
...
//当x为变量时因类型拓展为string而报错，当为常量时，类型为字面常量，故不会报错
let a:number = getComponent(vec, x);
console.log(a);
```
...
![1634261312(1).png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/cfdcc823c6ce4f96b87ebec63bcce2f1~tplv-k3u1fbpfcp-watermark.awebp?)

由上图可知，在混入多个类型时，若存在相同的成员，且**成员类型为非基本数据类型**，那么是可以成功合并。
...
来张图片帮助你理解 ![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ba87e7a8921144faa4e8dd7dc8e92916~tplv-k3u1fbpfcp-watermark.awebp?)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/dc2bcfb0e8414282b606705c6a0385e0~tplv-k3u1fbpfcp-watermark.awebp?)
...
```js
T[K]表示对象T的属性K所表示的类型，在下述例子中，T[K][] 表示变量T取属性K的值的数组
